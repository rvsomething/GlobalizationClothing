...
//   }
//   console.log(("user not in admin"));
//   const isPasswordMatched = await admin.comparePassword(password);
//   if (!isPasswordMatched) {
//     return next(new ErrorHandler('Invalid email or password 11', 401));
//   }
...
exports.updateAdminPrivilege = catchAsyncError(async (req, res, next) => {
  ...
  if (!privilege) {
    return next(new ErrorHandler('No privilege data provided', 400));
  }
  if (!['super', 'moderate', 'low'].includes(privilege)) {
    return next(new ErrorHandler('Invalid privilege level', 400));
  }
